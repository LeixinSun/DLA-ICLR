...
from .model_utils.misc import find_all_linear_modules, find_expanded_modules, find_sail_lora_target_modules
...
def _setup_lora_tuning(
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        # Handle SAIL layer-specific LoRA fine-tuning
        if (hasattr(finetuning_args, 'stage') and finetuning_args.stage == "sail" and 
            finetuning_args.finetuning_type == "lora" and 
            finetuning_args.lora_layer_range is not None):
            
            logger.info_rank0(f"SAIL LoRA fine-tuning with layer range: {finetuning_args.lora_layer_range}")
            
            if len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] == "all":
                # If lora_target is "all", use all available linear modules
                base_target_modules = find_all_linear_modules(model, finetuning_args.freeze_vision_tower)
            else:
                base_target_modules = finetuning_args.lora_target
            
            # Find specific modules based on layer range
            target_modules = find_sail_lora_target_modules(
                model, base_target_modules, finetuning_args.lora_layer_range, finetuning_args.freeze_vision_tower
            )
            
            if not target_modules:
                raise ValueError(f"No target modules found for the specified layer range: {finetuning_args.lora_layer_range}")
            
            logger.info_rank0(f"SAIL LoRA target modules: {len(target_modules)} modules selected")
            logger.info_rank0(f"First few target modules: {target_modules[:5]}")
